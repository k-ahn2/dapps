...
In 0.38.0: #178, two DAPPS neighbours dialling each other at the same moment. AX.25 has one link per callsign pair per port, and BPQ's AGW connect sends a fresh SABM down a link that is already up (`AGWAPI.c` issues a node-level connect without looking at existing links), which the far end's `L2Code.c` treats as a link reset that lands on the node prompt instead of `DAPPSv1>`; both sides then fail, requeue and redial in step. `PeerSessionRegistry` records which peers have a session open with us in either direction, fed by both inbound services and the outbound transport, and the forwarder leaves a message queued rather than dial into one (no cooldown, nothing counted against the route; a flood skips that neighbour). For the narrower case where both dial within one round trip and neither side is handed an inbound connect, `DappsProtocolClient` now tells total silence apart from a wrong banner, and after 60 s the lower callsign sends the prompt itself and serves the peer's session on the link it has with the ordinary `InboundConnectionHandler`, while the higher callsign keeps waiting and carries on as the caller. That send comes back as a `Deferred` result, neither success nor failure. Not covered: the scheduled poll and probe do not consult the registry yet.

Follow-up to #178 (#185): the forwarder's registry check ran once, early - before the settle-gate wait and the AGW/RHP connect round trip, both of which take real time - so a live capture still showed the glare pattern: an inbound session registered in that gap, after the check passed but before the outbound SABM reached BPQ. `PeerSessionRegistry.TryAcquire` closes it: `BearerSwitchingOutboundTransport` now checks-and-acquires atomically, under the registry's own lock, immediately before the dial rather than relying on the forwarder's earlier snapshot. A busy peer found there throws `PeerSessionBusyException`, which `Dappsv1SessionBackhaul` turns into the same `Deferred` result as any other #178 non-send. The forwarder's early check is kept as a fast path (skips the settle-gate wait and connect attempt outright in the common case); it is no longer the only guard.
